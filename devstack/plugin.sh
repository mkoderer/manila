...
function import_docker_service_image {
    download_image $MANILA_DOCKER_IMAGE_URL

    # Import image in Docker
    gzip -d $FILES/`basename "$MANILA_DOCKER_IMAGE_URL"`
    IMG_NAME=`basename "$MANILA_DOCKER_IMAGE_URL"`
    sudo docker load --input $FILES/${IMG_NAME%.*}
}
